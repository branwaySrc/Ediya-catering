import { CheckCircle2, ClipboardList, MessageCircle, PackageCheck, ShoppingCart } from "lucide-react";
...
const customOrderTools = [
	...
	{ label: "장바구니보기", description: "담은 메뉴와 수량을 확인합니다." },
	{ label: "숨기기", description: "하단 장바구니를 잠시 접고 메뉴를 계속 봅니다." },
	{ label: "비우기", description: "선택한 구성을 다시 처음부터 정리합니다." },
];
			...
			<ScreenSection>
				<div className="grid gap-10">
					<header className="max-w-3xl">
						<p className="inline-flex rounded-full border border-[#E96106]/20 bg-white px-4 py-2 text-base font-bold text-[#E96106]">CUSTOM ORDER GUIDE</p>
						<h2 id="custom-order-guide-heading" className="mt-5 text-3xl font-bold tracking-[-0.03em] text-slate-950 sm:text-4xl">
							장바구니가 상담 요청서가 됩니다.
						</h2>
						<p className="mt-5 text-base text-slate-600">
							커스텀 주문은 상품을 바로 결제하는 과정이 아니라, 필요한 메뉴와 수량을 담아 담당자에게 전달하는 상담형 주문 방식입니다.
						</p>
					</header>
					...
					<div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
						<article className="rounded-lg border border-[#E96106]/20 bg-white p-6">
							<div className="grid gap-4">
								<div className="max-w-[88%] rounded-lg border border-slate-200 bg-slate-50 p-4 text-base text-slate-700">아이스 아메리카노 20개와 베이커리 10개를 담았어요.</div>
								<div className="ml-auto max-w-[88%] rounded-lg bg-[#E96106] p-4 text-base font-bold text-white">장바구니 기준 예상 금액 확인 완료</div>
								<div className="max-w-[88%] rounded-lg border border-slate-200 bg-slate-50 p-4 text-base text-slate-700">이 구성으로 문의하면 바로 주문 확정인가요?</div>
								<div className="ml-auto max-w-[88%] rounded-lg bg-primary p-4 text-base font-bold text-white">
									담당자가 메뉴 가능 여부와 행사 조건을 확인한 뒤 상세 상담을 위해 연락드립니다.
								</div>
							</div>
						</article>

						<article className="rounded-lg border border-primary/10 bg-white p-6">
							<h3 className="text-2xl font-bold text-slate-950">장바구니 문의 흐름</h3>
							<ol className="mt-5 grid gap-3">
								{customOrderSteps.map(({ title, description, icon: Icon }, index) => (
									<li key={title} className="grid grid-cols-[auto_1fr] gap-3 rounded-lg border border-slate-200 bg-white p-4">
										...
										<div>
											<p className="text-base font-bold text-[#E96106]">STEP {String(index + 1).padStart(2, "0")}</p>
					...
					<div className="grid gap-5 lg:grid-cols-[0.95fr_1.05fr]">
						<article className="rounded-lg border border-[#E96106]/20 bg-white p-6">
							<h3 className="text-2xl font-bold text-slate-950">커스텀 주문이 잘 맞는 상황</h3>
						...
						<article className="rounded-lg border border-slate-200 bg-white p-6">
							<h3 className="text-2xl font-bold text-slate-950">장바구니 기능 안내</h3>
					...
					<article className="rounded-lg border border-primary/10 bg-white p-6">
						<h3 className="text-2xl font-bold text-slate-950">문의 전에 기억할 점</h3>
