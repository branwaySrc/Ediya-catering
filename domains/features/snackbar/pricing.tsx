		...
		<section className="bg-white py-20 sm:py-28" aria-labelledby="pricing-heading">
			<ScreenSection>
				<header className="mx-auto max-w-3xl text-center">
					...
					<h2 id="pricing-heading" className="mt-4 text-3xl font-bold tracking-[-0.03em] text-slate-900 sm:text-4xl">우리 팀에 맞는 월 예산 가이드</h2>
				...
				<div className="mt-12 grid gap-5">
					{pricePlans.map((plan, index) => (
						<article key={plan.id} className={`relative rounded-lg border p-6 sm:p-7 ${index === 1 ? "border-primary bg-[#F3F6FF] shadow-xl shadow-primary/10" : "border-slate-200 bg-white shadow-sm"}`}>
							...
							<p className="mt-5 text-sm font-bold text-[#1C5DBE]">{plan.people}</p>
							<h3 className="mt-1 text-3xl font-bold tracking-tight text-slate-900">{plan.monthlyBudget}</h3>
							<p className="mt-3 text-sm leading-6 text-slate-600">{plan.description}</p>
							<p className="mt-6 inline-flex rounded-full bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700">{plan.delivery}</p>
