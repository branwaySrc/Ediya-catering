...
export function CateringPackageCards() {
	return (
		<section className="bg-[#F8F9FC] py-16 sm:py-20" aria-labelledby="catering-package-cards-heading">
			<ScreenSection>
				<div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
					<div className="max-w-2xl">
						...
						<p className="hidden text-sm font-black uppercase tracking-[0.16em] text-primary/55">Packages</p>
						<h2 id="catering-package-cards-heading" className="mt-4 text-3xl font-black text-primary sm:text-4xl">
				...
				<div className="mt-10 grid gap-4 lg:grid-cols-3">
					{estimatePackages.map((item) => (
						<article key={item.id} className="flex flex-col rounded-3xl border border-primary/10 bg-white p-6 shadow-sm sm:p-7">
							<div className="flex items-start justify-between gap-4">
								<div>
									<p className="text-xs font-black uppercase tracking-[0.14em] text-primary/45">{item.name}</p>
									<h3 className="mt-2 text-2xl font-black text-primary">{item.koreanName}</h3>
								</div>
								<span className="inline-flex size-11 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-primary">
									<Users aria-hidden="true" className="size-5" />
								</span>
							</div>

							<p className="mt-5 text-2xl font-black text-primary">1인 {priceFormatter.format(item.pricePerPerson)}원부터</p>
							<p className="mt-4 min-h-16 text-sm leading-7 text-slate-600">{item.description}</p>

							<div className="mt-6">
								<p className="text-sm font-black text-primary">추천 대상</p>
