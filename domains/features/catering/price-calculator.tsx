...
export function CateringPriceCalculator() {
	...
	return (
		<section id="estimate" className="scroll-mt-24 bg-white py-16 sm:py-20" aria-labelledby="catering-calculator-heading">
			<ScreenSection>
				<div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
					<div>
						...
						<p className="hidden text-sm font-black uppercase tracking-[0.16em] text-primary/55">Estimate Calculator</p>
						<h2 id="catering-calculator-heading" className="mt-4 text-3xl font-black leading-tight text-primary sm:text-4xl">
							예상 인원으로
							<br />
							빠르게 견적을 확인하세요
						</h2>
					...
					<div className="rounded-3xl border border-primary/10 bg-[#F8F9FC] p-5 shadow-xl shadow-primary/5 sm:p-6 lg:p-8">
							...
							{estimatePackages.map((item) => {
								...
								return (
									<button
										...
										className={`relative rounded-2xl border p-4 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 ${
									...
									>
										<p className={`text-xs font-black uppercase tracking-[0.12em] ${isSelected ? "text-white/60" : "text-primary/45"}`}>{item.name}</p>
										<h3 className="mt-2 text-base font-black">{item.koreanName}</h3>
										<p className={`mt-3 text-sm font-black ${isSelected ? "text-white" : "text-primary"}`}>1인 {priceFormatter.format(item.pricePerPerson)}원부터</p>
						...
						<div className="mt-6 grid gap-5 lg:grid-cols-[0.85fr_1.15fr]">
							<label className="block rounded-2xl border border-primary/10 bg-white p-5">
								<span className="text-sm font-black text-primary">예상 인원</span>
								<div className="mt-4 flex items-center gap-3">
									<input
										...
										className="h-14 w-full rounded-xl border border-slate-200 bg-white px-4 text-2xl font-black text-primary outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/15"
										aria-describedby="headcount-help"
									/>
									<span className="shrink-0 text-lg font-black text-primary">명</span>
								</div>
							...
							<div className="rounded-2xl bg-primary p-6 text-white" aria-live="polite">
								<div className="flex items-center justify-between gap-4">
									<div>
										<p className="text-xs font-black uppercase tracking-[0.14em] text-white/55">{selectedPackage.name}</p>
										<h3 className="mt-2 text-xl font-black">{selectedPackage.koreanName} 기준</h3>
									</div>
								...
								<p className="mt-1 text-4xl font-black tracking-tight">{priceFormatter.format(totalPrice)}원</p>
