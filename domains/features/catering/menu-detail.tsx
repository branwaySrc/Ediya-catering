		...
		<div className="bg-[#F8F9FC] text-slate-900">
			<section className="bg-white py-16 sm:py-20">
				<ScreenSection className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
					...
					<div className="rounded-3xl border border-primary/10 bg-primary p-6 text-white shadow-xl shadow-primary/15 sm:p-8">
						<span className="inline-flex size-12 items-center justify-center rounded-2xl bg-white/10">
							<Search aria-hidden="true" className="size-6" />
						</span>
						<h2 className="mt-6 text-2xl font-black">카테고리별 확인 후 바로 문의</h2>
						<p className="mt-3 text-sm leading-7 text-blue-100/75">
							원하는 메뉴군이 있으면 문의 링크에 카테고리를 함께 전달합니다. 상담 시 메뉴 구성과 수량을 더 빠르게 정리할 수 있습니다.
						</p>
						<Button.Link href="/contact?service=catering&type=menu" variant="outlined" icon={ArrowRight} iconPosition="right" className="mt-7 border-white bg-white text-primary hover:bg-blue-50">
							메뉴 상담하기
						</Button.Link>
					</div>
				</ScreenSection>
			...
			<section className="py-16 sm:py-20">
				<ScreenSection className="grid gap-6">
					{detailedMenuCategories.map((category) => (
						<article key={category.id} id={category.id} className="scroll-mt-40 overflow-hidden rounded-3xl border border-primary/10 bg-white shadow-sm">
							<div className="grid lg:grid-cols-[0.72fr_1.28fr]">
								...
								<div className="p-6 sm:p-8">
									<div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
										<div>
											<p className="text-xs font-black uppercase tracking-[0.16em] text-primary/45">{category.id}</p>
											<h2 className="mt-2 text-2xl font-black text-primary">{category.name}</h2>
