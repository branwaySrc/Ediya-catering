...
export function GiftMomentIntro() {
	return (
		...
		<section className="py-20 sm:py-28">
			<ScreenSection>
				...
				<div className="mt-8 grid gap-4 rounded-lg bg-[#102B24] p-6 text-white sm:p-8 lg:grid-cols-[auto_1fr_auto] lg:items-center">
					...
					<Link href={routes.gift.news} className="inline-flex items-center gap-2 text-sm font-bold text-[#91E2BD]">
						최신 구성 문의하기
						<ArrowRight aria-hidden="true" className="size-4" />
					</Link>
				</div>
				...
				<div className="relative overflow-hidden rounded-lg bg-[#153D32] px-6 py-12 text-white sm:px-12 sm:py-16 lg:px-16 lg:py-20">
					...
					<div className="relative grid gap-10 lg:grid-cols-[1fr_0.85fr] lg:items-end">
						<div className="max-w-2xl">
							...
							<p className="mt-5 text-sm leading-7 text-emerald-50/75 sm:text-base">시즌마다 달라지는 최신 MD 상품과 단체 선물 구성을 카카오톡 또는 이메일로 안내해 드립니다.</p>
						</div>

						<div className="grid gap-3">
							<Link href={KAKAO_CHANNEL_URL} className="inline-flex min-h-14 items-center justify-between gap-4 rounded-md bg-[#FEE500] px-6 text-sm font-bold text-[#191919] transition hover:bg-[#F4DC00]">
							...
							<a href={emailSubscriptionHref} className="inline-flex min-h-14 items-center justify-between gap-4 rounded-md border border-white/25 bg-white/10 px-6 text-sm font-bold text-white transition hover:bg-white/15">
								<span className="inline-flex items-center gap-3">
							...
							<p className="px-1 text-xs leading-5 text-emerald-50/55">이메일 버튼을 누르면 구독 신청 내용이 입력된 메일 작성 화면이 열립니다. 카카오톡 채널 주소는 실제 채널 링크로 교체할 수 있습니다.</p>
						</div>
					</div>
				</div>
			</ScreenSection>
